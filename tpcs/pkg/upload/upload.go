...
func (us *UploadStatus) Write(p []byte) (int, error) {
	...
	err := Session.Save()
	if err != nil {
		logger.Errorf("session.Save() error: %v\n", err)
		return 0, err
	}
...
func SaveFile(item int, file *multipart.FileHeader, dst string) error {
	...
	if err != nil {
		logger.Errorf("io.Copy(out, io.TeeReader(src, uploadStatus)) err: %v", err)
		return err
	}
	return nil
}
